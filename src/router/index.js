...
router.afterEach((to) => {
  ...
  if (to.name == "RentPage") {
    ...
    isTitle = `Apartments For Rents in ${slugParam} | Real Buy`;
    ...
    isTitle = `${slugParam} Recently Sold Properties | Real Buy`;
    makeActiveRouteToBe = to.name;
    setRouteMetaTitle = !zipCode ? isTitle : isZipCodeTitle;
  } else if (to.name == "BuyPage") {
    isZipCodeTitle = `Homes For Sale & Real Estate in ${to.params.zipCode} Zip Code`;
    isTitle = `${slugParam} Homes For Sale & ${slugParam} Real Estate | Real Buy`;
    makeActiveRouteToBe = to.name;
    setRouteMetaTitle = !zipCode ? isTitle : isZipCodeTitle;
  } else if (to.name == "RentsNearMe") {
    makeActiveRouteToBe = "RentPage";
    setRouteMetaTitle = `Apartments for rent near me - Find An Apartment Nearby | Real Buy`;
  } else if (to.name == "SalesNearMe") {
    setRouteMetaTitle = `Houses for sale near me - Find a nearby Real Estate & Homes | Real Buy`;
    makeActiveRouteToBe = "BuyPage";
  } else if (to.name == "SalesPageDetail" || to.name == "RentPageDetail") {
    setRouteMetaTitle = `${addressParam} in ${cityParam}, ${to.params.slug} - ${to.params.propertyId} | Real Buy`;
    makeActiveRouteToBe = to.name;
  } else {
